...
// v2.52F: matching base/lid version marks, lid TOP/BOTTOM marks, and visible Pi GPIO pin 1/40 markers.
// Lid text overlaps the lid underside by 0.05mm so it is fused into the printable geometry.
// GPIO 1 is on the base floor, clear of the Pi and case wall.
// Speaker update:
...
// 38mm mount centre spacing gives 5.10mm clearance from each mount edge to the inside wall
...
// TTP223B touch sensor added to the wall opposite the four rubber feet.
// PCB footprint: 24mm x 24mm with a 20mm x 20mm M2.5 mounting pattern.
// The 26mm x 26mm touch panel is recessed from inside to leave a 2.5mm local wall.
// A 14mm circular recess on the outside leaves exactly 1.0mm of plastic over the sensing electrode.
// The PCB rests directly against the local panel's flat inside face.
// The complete touch assembly is positioned toward the far-right side of the case.
// The rightmost 14mm of the PCB is the sensing electrode; the leftmost 10mm holds components.
// The touch target is a recessed 14mm circle aligned with the sensing electrode.
// An 8mm x 3mm blind pocket on the PCB left side gives 1.7mm solder-joint clearance while retaining a slicer-safe 0.8mm solid exterior skin.
// The slot is moved 1.2mm farther inward from its previous position.
// Four 5.5mm shallow bolt-head seats accommodate M2.5 heads up to 5.0mm diameter.
// Units: mm
...
speaker_mount_span = 38.0; // center-to-center distance between speaker mount pilot holes
speaker_mount_center_to_wall = (case_h - 2 * wall - speaker_mount_span) / 2; // 8.0mm from inside wall to mount center
speaker_mount_wall_clearance = speaker_mount_center_to_wall - speaker_mount_w / 2; // 5.10mm gap from mount tab edge to inside wall
...
// Matching part numbers on the base and lid.
base_part_number = "MK-PiClock-V2.52F";
...
lid_part_number = base_part_number;
...
lid_top_text_y = 52.0;
lid_bottom_text_x = case_w / 2;
lid_bottom_text_y = 6.8;

// Physical GPIO end markers beside the Pi header.
// The Pi is rotated 180 degrees in this enclosure:
// - pin 40 is at the left end of the header
// - pin 1 is at the right end
// Both labels are raised on exposed base-floor areas so the Pi cannot hide them.
gpio_mark_size = 3.2;
gpio_mark_h = 0.55;

gpio_40_mark = "40";
gpio_40_mark_x = pi_x - 3.0;
gpio_40_mark_y = pi_y + 5.0;

gpio_1_mark = "1";
gpio_1_mark_x = pi_x + pi_w - 5.0;
gpio_1_mark_y = pi_y - 2.2;
...
// Mounted on the wall opposite the four rubber feet, which faces the room ceiling.
// The complete touch assembly is moved toward the far-right side of the case.
// The 24mm PCB remains 5mm left of its 14mm sensing electrode so the right-side
// sensing area aligns with the recessed circle. The selected X position leaves
// clearance from the upper-right lid support and the Pi USB connector envelope.
touch_pcb_size = 24.0;
touch_pcb_t = 1.6;
touch_hole_span = 20.0;
touch_m25_clearance_d = 2.9;

// M2.5 bolt-head allowance. A 5.5mm seat gives 0.25mm radial clearance
// around a head measuring up to 5.0mm. The seat is intentionally shallow.
...
// Actual sensing electrode geometry.
// The rightmost 14mm of the 24mm PCB is the sensing area. The remaining
// 10mm on the left contains non-sensing PCB components. The full assembly
// is positioned far right while retaining clearance from the right lid mount.
touch_pad_d = 14.0;
touch_component_strip_w = touch_pcb_size - touch_pad_d; // 10mm
touch_pcb_offset_x = touch_component_strip_w / 2;       // 5mm

// PCB/wall-pad centre. This places the 26mm wall pad from X=74mm to X=100mm.
// The 14mm sensing recess is centred at X=92mm and spans X=85mm to X=99mm.
touch_center_x = 87.0;
touch_pad_center_x = touch_center_x + touch_pcb_offset_x;
touch_pad_center_z = touch_center_z;

// Solder-joint envelope on the wall-facing side of the PCB.
// The 3mm dimension runs inward from the PCB left edge and the 8mm dimension
// runs vertically. Its centre is 2.7mm inward from the left PCB edge.
touch_solder_x_size = 3.0;
touch_solder_z_size = 8.0;
touch_solder_center_from_left_edge = 2.7;
touch_solder_clearance = 1.7;
touch_solder_center_x = touch_center_x - touch_pcb_size / 2 + touch_solder_center_from_left_edge;
touch_solder_center_z = touch_center_z;

// Thin, flush local touch panel. A 0.8mm exterior skin is wide enough for
// normal 0.4mm-nozzle slicer line widths while preserving 1.7mm solder clearance.
// The PCB therefore sits 0.4mm farther inward than the previous 2.1mm panel.
touch_panel_size = 26.0;
touch_solder_floor_thickness = 0.8;
touch_wall_thickness = touch_solder_clearance + touch_solder_floor_thickness; // 2.5mm total
touch_panel_inner_y = case_h - touch_wall_thickness;
touch_pcb_outer_face_y = touch_panel_inner_y;

// Outside circular touch recess. The 2.5mm panel is reduced to a 1.0mm membrane
// only over the 14mm sensing electrode.
touch_sensing_skin_thickness = 1.0;
touch_circle_recess_d = touch_pad_d;
touch_circle_recess_depth = touch_wall_thickness - touch_sensing_skin_thickness; // 1.5mm
...
module gpio_marker_text() {
    // Pin 40: left of the Pi footprint.
    translate([
        gpio_40_mark_x,
        gpio_40_mark_y,
        wall
        ...
        rotate([0, 0, 90])
            linear_extrude(height = gpio_mark_h)
                text(
                    gpio_40_mark,
                    size = gpio_mark_size,
                    halign = "center",
    ...
    // Pin 1: on the exposed floor below the Pi's right GPIO end, not on the wall.
    translate([
        gpio_1_mark_x,
        gpio_1_mark_y,
        wall
...
module touch_sensor_panel_recess() {
    // Remove material from the inside of the original 3mm shell wall,
    // leaving a uniform 2.5mm local wall across the 26mm square panel.
    translate([
        touch_center_x - touch_panel_size / 2,
        case_h - wall - 0.10,
        touch_center_z - touch_panel_size / 2
    ])
        cube([
            touch_panel_size,
            wall - touch_wall_thickness + 0.10,
            touch_panel_size
        ]);
}

module touch_sensor_solder_pocket() {
    // 8mm x 3mm blind pocket from the PCB face toward the exterior.
    // The 1.7mm pocket leaves a continuous 0.8mm slicer-safe exterior skin.
    translate([
...
module touch_sensor_mount_holes() {
    // M2.5 clearance holes pass through the 2.5mm touch panel.
    for (dx = [-touch_hole_span / 2, touch_hole_span / 2]) {
        for (dz = [-touch_hole_span / 2, touch_hole_span / 2]) {
            translate([
                touch_center_x + dx,
                touch_panel_inner_y - 1,
                touch_center_z + dz
            ])
                rotate([-90, 0, 0])
                    cylinder(
                        h = touch_wall_thickness + 2,
...
module touch_sensor_head_seats() {
    // Shallow outside seats locate M2.5 heads up to 5.0mm diameter.
    for (dx = [-touch_hole_span / 2, touch_hole_span / 2]) {
        for (dz = [-touch_hole_span / 2, touch_hole_span / 2]) {
            translate([
                touch_center_x + dx,
...
module touch_sensor_circle_recess() {
    // Circular pocket cut inward from the exterior face.
    // The remaining flat membrane over the electrode is exactly 1.0mm thick.
    translate([
        touch_pad_center_x,
        case_h + 0.05,
        touch_pad_center_z
...
module base_cutouts() {
    ...
    touch_sensor_panel_recess();
    touch_sensor_mount_holes();
    touch_sensor_head_seats();
    touch_sensor_solder_pocket();
    touch_sensor_circle_recess();
...
module touch_sensor_preview() {
    ...
    // 24mm PCB rests directly against the recessed wall panel.
    color("green", 0.35)
        translate([
            ...
            cube([touch_pcb_size, touch_pcb_t, touch_pcb_size]);

    // Actual 14mm sensing electrode preview on the PCB right side.
    color("gold", 0.40)
        translate([
            touch_pad_center_x,
    ...
    // Solder-joint envelope passes into the 1.7mm through-slot.
    color("silver", 0.55)
        translate([
            touch_solder_center_x - touch_solder_x_size / 2,
            touch_panel_inner_y,
    ...
    // Four M2.5 shafts and 5.0mm heads, without separate mounting bosses.
    color("silver", 0.45)
        for (dx = [-touch_hole_span / 2, touch_hole_span / 2]) {
            for (dz = [-touch_hole_span / 2, touch_hole_span / 2]) {
                translate([
                    touch_center_x + dx,
                    touch_panel_inner_y - touch_pcb_t - 0.8,
                    touch_center_z + dz
                ])
                    rotate([-90, 0, 0])
                        cylinder(
                            h = touch_wall_thickness + touch_pcb_t + 1.6,
                            d = 2.5
                        );

                translate([
                    touch_center_x + dx,
                    case_h + touch_m25_head_h - touch_m25_head_seat_depth,
                    touch_center_z + dz
                ])
                    rotate([90, 0, 0])
                        cylinder(h = touch_m25_head_h, d = 5.0);
            }
        }
}
