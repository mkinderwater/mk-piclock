...
base_part_number = "MK-PiClock-V2.51";
...
module lid_lip_corner_mount_relief(p, z0, zh) {
    ...
    if (p[0] < case_w / 2 && p[1] < case_h / 2) {
        translate([
            wall - lid_mount_wall_overlap - lid_mount_lip_clearance,
            wall - lid_mount_wall_overlap - lid_mount_lip_clearance,
            z0
        ])
            cube([
    ...
    difference() {
        ...
        for (p = lid_points()) {
            ...
            translate([
                p[0],
                p[1],
                lid_t - lid_screw_head_recess_h
            ])
                cylinder(
                    h = lid_screw_head_recess_h + 1,
                    d = lid_screw_head_d
                );
        }
    }
